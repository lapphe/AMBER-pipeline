...
class UserDefinedFeatureExtractor(ConfigReader, FeatureExtractionMixin):
    ...
    def extract_features(self):
        ...
        for file_cnt, file_path in enumerate(self.files_found):
            ...
            csv_df['pups_centroid_x_roll_mean_30m'] = csv_df['pups_centroid_mult_x'].rolling(roll_rows_30, min_periods=0, center=True).sum() / \
                                                        csv_df['high_p_pup_bp_no_zero'].rolling(roll_rows_30, min_periods=0, center=True).sum()
            csv_df['pups_centroid_y_roll_mean_30m'] = csv_df['pups_centroid_mult_y'].rolling(roll_rows_30, min_periods=0, center=True).sum() / \
                                                        csv_df['high_p_pup_bp_no_zero'].rolling(roll_rows_30, min_periods=0, center=True).sum()
            
            roll_rows_60 = int(60*60*fps)
            csv_df['pups_centroid_x_roll_mean_60m'] = csv_df['pups_centroid_mult_x'].rolling(roll_rows_60, min_periods=0, center=True).sum() / \
                                                        csv_df['high_p_pup_bp_no_zero'].rolling(roll_rows_60, min_periods=0, center=True).sum()
            csv_df['pups_centroid_y_roll_mean_60m'] = csv_df['pups_centroid_mult_y'].rolling(roll_rows_60, min_periods=0, center=True).sum() / \
                                                        csv_df['high_p_pup_bp_no_zero'].rolling(roll_rows_60, min_periods=0, center=True).sum()
            ...
            csv_df['dam_pup30m_distance'] = np.sqrt((csv_df['dam_centroid_x'] - csv_df['pups_centroid_x_roll_mean_30m']) ** 2 +
                                                 (csv_df['dam_centroid_y'] - csv_df[
                                                     'pups_centroid_y_roll_mean_30m']) ** 2) / pixels_per_mm
            csv_df['head_pup30m_distance'] = np.sqrt((csv_df['head_centroid_x'] - csv_df['pups_centroid_x_roll_mean_30m']) ** 2 +
                                                  (csv_df['head_centroid_y'] - csv_df[
                                                      'pups_centroid_y_roll_mean_30m']) ** 2) / pixels_per_mm
                                                                                         
            csv_df['dam_pup60m_distance'] = np.sqrt((csv_df['dam_centroid_x'] - csv_df['pups_centroid_x_roll_mean_60m']) ** 2 +
                                                 (csv_df['dam_centroid_y'] - csv_df[
                                                     'pups_centroid_y_roll_mean_60m']) ** 2) / pixels_per_mm
            csv_df['head_pup60m_distance'] = np.sqrt((csv_df['head_centroid_x'] - csv_df['pups_centroid_x_roll_mean_60m']) ** 2 +
                                                  (csv_df['head_centroid_y'] - csv_df[
                                                      'pups_centroid_y_roll_mean_60m']) ** 2) / pixels_per_mm
            ...
            csv_df['high_p_pup_bp_roll_sum_1s'] = csv_df['high_p_pup_bp'].rolling(roll_windows[0], min_periods=0, center=True).sum()
            csv_df['high_p_pup_bp_roll_sum_1ds'] = csv_df['high_p_pup_bp'].rolling(roll_windows[2], min_periods=0, center=True).sum()
            csv_df['high_p_pup_bp_roll_sum_2s'] = csv_df['high_p_pup_bp'].rolling(roll_windows[4], min_periods=0, center=True).sum()

            csv_df['high_p_dam_bp_roll_sum_1s'] = csv_df['high_p_dam_bp'].rolling(roll_windows[0], min_periods=0, center=True).sum()
            csv_df['high_p_dam_bp_roll_sum_1ds'] = csv_df['high_p_dam_bp'].rolling(roll_windows[2], min_periods=0, center=True).sum()
            csv_df['high_p_dam_bp_roll_sum_2s'] = csv_df['high_p_dam_bp'].rolling(roll_windows[4], min_periods=0, center=True).sum()
            ...
            csv_df['high_p_pup_bp_roll_std_1s'] = csv_df['high_p_pup_bp'].rolling(roll_windows[0], min_periods=0, center=True).std()
            csv_df['high_p_pup_bp_roll_std_2s'] = csv_df['high_p_pup_bp'].rolling(roll_windows[4], min_periods=0, center=True).std()

            csv_df['high_p_dam_bp_roll_std_1s'] = csv_df['high_p_dam_bp'].rolling(roll_windows[0], min_periods=0, center=True).std()
            csv_df['high_p_dam_bp_roll_std_2s'] = csv_df['high_p_dam_bp'].rolling(roll_windows[4], min_periods=0, center=True).std()
